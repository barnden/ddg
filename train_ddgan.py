...
from datasets_prep.shapenet import ShapenetDataset
from datasets_prep.ffhq import ImageFolderDataset
from datasets_prep.fakeshape import DebugDataset
from triplane.camera import get_relative, random_lookat, create_lookat_extrinsics

from torch.multiprocessing import Process
import torch.distributed as dist
import shutil

from triplane.network import PoseAwareDiscriminator
...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        ...
        for i in reversed(range(n_time)):
            ...
            x_0 = generator(x, t_time, latent_z, c)["image"]
...
def train(rank, gpu, args):
    ...
    # from triplane.network import TriNCSNpp
    ...
    if args.dataset == 'cifar10':
        ...
        dataset = ShapenetDataset(root='./datasets/pxfcars-lmdb', name='shapenet_cars', transform=train_transform)
        ...
        dataset = ImageFolderDataset('/DATA/FFHQ_512.zip', None, use_labels=True, transform=train_transform)
        ...
        dataset = DebugDataset(root='./datasets/celeba-lmdb', name='celeba', train=True, transform=train_transform)
    else:
        raise NotImplementedError(f"Support for \"{args.dataset}\" dataset has not been implemented")
    ...
    rendering_options = {
        ...
        'decoder_lr_mul': 0, # FIXME: Implement learning rate multiplier for linear layer
    }
    ...
    elif args.dataset in ['ffhq', 'cars', 'debug']:
        netD = PoseAwareDiscriminator(nc = 2*args.num_channels, ngf = args.ngf,
                               t_emb_dim = args.t_emb_dim,
                               act=nn.LeakyReLU(0.2)).to(device)
    else:
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (c, x, y) in enumerate(data_loader):
            ...
            # train with real
            D_real = netD(x_t, t, x_tp1.detach(), c).view(-1)
            ...
            embed_pose = c.clone() # Pose to embed into diffusion
            render_pose = c.clone() # Pose to render output triplane at
            discriminator_pose = c.clone() # Pose to pass to discriminator

            # Exp 1: pose: embed = render = discriminator = absolute
            # Exp 2: no pose embed in diffusion, pose: render = discriminator = absolute
            # Exp 3: pose: embed = absolute, render = random - absolute, discriminator = random
            # Exp 4: no pose anywhere

            if args.relative_render:
                ...
                if args.random_render:
                    ...
                    discriminator_pose = c


            x_0_predict = netG(x_tp1.detach(), t, latent_z, render_pose, embed_pose)['image']
            x_pos_sample = sample_posterior(pos_coeff, x_0_predict, x_tp1, t)

            output = netD(x_pos_sample, t, x_tp1.detach(), discriminator_pose).view(-1)
            ...
            #     Path("./saved_info/discr").mkdir(exist_ok=True)
            #     tvu.save_image(torch.cat([x_0_predict, x_pos_sample, x_tp1, x_t]), "./saved_info/discr/car_{}_{}_{}.png".format(epoch, iteration, t[0].item()))
            ...
            # errG = (x_pos_sample - x_t) ** 2
            # errG = errG.mean(dim=list(range(1, len(errG.shape))))
